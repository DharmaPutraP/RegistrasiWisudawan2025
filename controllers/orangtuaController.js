...
export const updateOrangtuaKonsumsi = async (req, res) => {
  try {
    if (req.enabledFeatures.Konsumsi) {
      ...
      res.status(StatusCodes.OK).json({
        msg: "Orangtua Konsumsied modified",
        orangtua: updatedOrangtua,
      });
    } else {
      res
        .status(StatusCodes.NOT_ACCEPTABLE)
        .json({ message: "Pengambilan Konsumsi ditutup!" });
    }
  } catch (error) {
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
...
export const updateOrangtuaRegister = async (req, res) => {
  try {
    if (!req.enabledFeatures.Registrasi) {
      return res
        .status(StatusCodes.NOT_ACCEPTABLE)
        .json({ message: "Registrasi ditutup!" });
    }
    let isRegis = true;
    ...
    //       let nomorTerakhir = sortedNoKursiOrangtua[0].noKursi.slice(
    //         getPintu.code.length
    //       ); // Ambil bagian nomor saja
    ...
    //         const aNum = parseInt(
    //           a.noKursi.slice(getAllPintu[0].code.length),
    //           10
    //         );
    //         const bNum = parseInt(
    //           b.noKursi.slice(getAllPintu[0].code.length),
    //           10
    //         );
    //         return bNum - aNum;
    //       });
    ...
    //         let nomorTerakhirBaru = sortedNoKursiOrangtuaBaru[0].noKursi.slice(
    //           getAllPintu[0].code.length
    //         );
    //         setPintu = `${getAllPintu[0].code}${
    //           parseInt(nomorTerakhirBaru, 10) + 1
    //         }`;
    //       } else {
